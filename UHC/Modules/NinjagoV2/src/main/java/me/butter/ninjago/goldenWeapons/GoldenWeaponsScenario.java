...
public class GoldenWeaponsScenario extends AbstractScenario {
    ...
    private class SpawnRunnable extends BukkitRunnable {
        public SpawnRunnable() {
            this.runTaskTimer(NinjagoV2.getInstance(), 0, timeBetweenSpawn * 20L);
        }

        @Override
        public void run() {
            if(NinjagoV2.getInstance().getGoldenWeaponManager().getChests().size() >= NinjagoV2.getInstance().getGoldenWeaponManager().getWeapons().size()) {
                cancel();
                return;
            }
            ...
            NinjagoV2.getInstance().getGoldenWeaponManager().addChest(structure);
