...
public class OrePopulator extends BlockPopulator {
    ...
    public int randInt(final Random rand, final int min, final int max) {
    ...
    public void populate(final World world, final Random random, final Chunk chunk) {
        ...
        for (final Rule bloc : this.rules) {
            for (int i = 0; i < bloc.amount; ++i) {
                ...
                this.generateVein(world, random, x, y, z, bloc.size, bloc);
            }
        }
    }

    public void generateVein(final World world, final Random rand, final int x, final int y, final int z, final int size, final Rule material) {
        ...
        for (int i = 0; i <= size; ++i) {
            ...
            for (int ix = xStart; ix <= xEnd; ++ix) {
                ...
                if (xThresh * xThresh >= 1.0) return;

                for (int iy = yStart; iy <= yEnd; ++iy) {
                    final double yThresh = (iy + 0.5 - yPos) / (fuzzY / 2.0);
                    if (xThresh * xThresh + yThresh * yThresh >= 1.0) return;

                    for (int iz = zStart; iz <= zEnd; ++iz) {
                        final double zThresh = (iz + 0.5 - zPos) / (fuzzXZ / 2.0);
                        if (xThresh * xThresh + yThresh * yThresh + zThresh * zThresh >= 1.0) return;
    ...
    public static class Rule {
        ...
        public Rule(final Material type, final int amount, final int minY, final int maxY, final int size) {
            this.id = type;
            this.amount = amount;
            this.minY = minY;
            this.maxY = maxY;
